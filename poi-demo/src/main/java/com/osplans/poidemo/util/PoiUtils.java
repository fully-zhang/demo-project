package com.osplans.poidemo.util;
...
public class PoiUtils {
    ...
    public PoiUtils(Workbook workboook) {
        this.workbook = workboook;
    }

    public PoiUtils(InputStream is, String version) throws FileNotFoundException, IOException {
    ...
    public void write(int sheetIx, List<List<String>> rowData, int startRow, OutputStream outputStream) throws IOException {
        Sheet sheet = workbook.getSheetAt(sheetIx);
        int dataSize = rowData.size();
        if (getRowCount(sheetIx) > 0) {// 如果小于等于0，则一行都不存在
            sheet.shiftRows(startRow, getRowCount(sheetIx), dataSize);
